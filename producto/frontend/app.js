...
// No existen llamadas HTTP ni conexiones reales con Odoo o UBLHUB.
...
const baseOrders = [
  { id: "PED-1006", customer: "Cliente demo 06", channel: "ERP/presencial", sku: "DEMO-001", qty: 1, status: "Pendiente", invoice: "Sin emitir", date: "2026-09-24" },
  { id: "PED-1005", customer: "Cliente demo 05", channel: "WhatsApp móvil", sku: "DEMO-002", qty: 2, status: "En preparación", invoice: "Borrador", date: "2026-09-24" },
  { id: "PED-1004", customer: "Cliente demo 04", channel: "ERP/presencial", sku: "DEMO-003", qty: 1, status: "Completado", invoice: "Simulado", date: "2026-09-23" },
  { id: "PED-1003", customer: "Cliente demo 03", channel: "ERP/presencial", sku: "DEMO-001", qty: 1, status: "Completado", invoice: "Simulado", date: "2026-09-23" },
  { id: "PED-1002", customer: "Cliente demo 02", channel: "WhatsApp móvil", sku: "DEMO-001", qty: 2, status: "En preparación", invoice: "Borrador", date: "2026-09-22" },
  { id: "PED-1001", customer: "Cliente demo 01", channel: "ERP/presencial", sku: "DEMO-002", qty: 1, status: "Completado", invoice: "Simulado", date: "2026-09-22" },
];
...
function demoStrip() { return `<div class="demo-strip"><span class="strip-icon">ⓘ</span><strong>Prototipo visual</strong><span>Los datos son ficticios. Los cambios se guardan solo en este navegador; no hay conexión con Odoo ni UBLHUB.</span></div>`; }
...
function renderInvoices() {
  ...
  return `${header("Documentos electrónicos", "UBLHUB", "Previsualización de boletas, facturas y guías; no emite documentos reales.")}${demoStrip()}<div class="toolbar"><label class="search-wrap"><span class="sr-only">Buscar documentos</span><input id="search-input" type="search" placeholder="Buscar pedido o cliente" value="${safe(query)}" /></label></div><section class="panel"><div class="panel-head"><div><h2>Documentos de demostración</h2><p>El estado “Simulado” solo representa una pantalla del flujo.</p></div><span class="badge gray">Sin conexión UBLHUB</span></div><div class="table-scroll"><table class="data-table"><thead><tr><th>Referencia</th><th>Cliente demo</th><th>Tipo propuesto</th><th>Importe</th><th>Estado visual</th><th></th></tr></thead><tbody>${rows.length ? rows.map((o) => `<tr><td><span class="table-main">${safe(o.id)}</span><span class="table-sub">${dateText(o.date)}</span></td><td>${safe(o.customer)}</td><td>Boleta / factura / guía</td><td>${money(orderTotal(o))}</td><td>${badge(o.invoice)}</td><td><button class="text-button" type="button" data-action="detail" data-id="${safe(o.id)}">Ver detalle →</button></td></tr>`).join("") : `<tr><td colspan="6"><div class="empty"><strong>Sin resultados</strong>Prueba otra búsqueda.</div></td></tr>`}</tbody></table></div></section><div class="note-box">La emisión, numeración, respuestas, archivos y reglas tributarias quedan fuera de este frontend. Se probarán mediante el sandbox de UBLHUB cuando exista backend y autorización.</div>`;
}
function renderChannels() {
  return `${header("Integraciones", "Canales de venta", "Dos canales convergen en Odoo y los documentos se procesan mediante UBLHUB.")}${demoStrip()}<div class="info-grid"><div class="info-card"><span class="mini-icon">◇</span><strong>ERP/presencial</strong><p>El vendedor registra directamente al cliente y su pedido en Odoo.</p></div><div class="info-card"><span class="mini-icon">▤</span><strong>WhatsApp móvil</strong><p>La atención es manual y el vendedor traslada el pedido a Odoo con su canal de origen.</p></div><div class="info-card"><span class="mini-icon">▦</span><strong>UBLHUB</strong><p>Microservicio seleccionado para boletas, facturas y guías de remisión.</p></div></div><section class="panel"><div class="panel-head"><div><h2>Recorrido propuesto</h2><p>Ambos canales comparten clientes, ventas e inventario en Odoo.</p></div></div><div class="flow-strip"><div class="flow-node"><strong>ERP o WhatsApp</strong><small>Origen del pedido</small></div><span class="flow-arrow">→</span><div class="flow-node"><strong>Ventas Odoo</strong><small>Cliente y pedido</small></div><span class="flow-arrow">→</span><div class="flow-node"><strong>Inventario Odoo</strong><small>Disponibilidad</small></div><span class="flow-arrow">→</span><div class="flow-node"><strong>UBLHUB</strong><small>Documento electrónico</small></div></div></section><div class="note-box">Esta pantalla es una simulación: no sincroniza WhatsApp, Odoo ni UBLHUB.</div>`;
}
